...
|------|------------|----|-----------|-----------------------|
| 2026-05-21 | 17:02 | CEPR-GOV-HARDENING-02 | Hardening adicional de governança: `scout-preview-smoke` sem `paths` no gatilho, detecção de escopo intra-job, `CODEOWNERS`, guardião automático de evidências de PR, branch protection reforçado e pin de versão do Supabase CLI para remover falha por rate limit | `.github/workflows/scout-preview-smoke.yml` atualizado · `.github/CODEOWNERS` criado · `.github/workflows/pr-evidence-guard.yml` criado · `.github/workflows/{scout-contract-cepr0098d,supabase-foundation,athlete-auth-foundation}.yml` com `supabase-cli 2.98.1` · `gh api .../required_pull_request_reviews` com `require_code_owner_reviews=true` e `require_last_push_approval=true` · `npm run typecheck` ✅ · `npm run build` ✅ |

...
|------|------------|----|-----------|-----------------------|
| 2026-05-21 | 17:02 | CEPR-GOV-HARDENING-02 | Hardening adicional de governança: `scout-preview-smoke` sem `paths` no gatilho, detecção de escopo intra-job, `CODEOWNERS`, guardião automático de evidências de PR e branch protection reforçado para code owner + aprovação do último push | `.github/workflows/scout-preview-smoke.yml` atualizado · `.github/CODEOWNERS` criado · `.github/workflows/pr-evidence-guard.yml` criado · `gh api .../required_pull_request_reviews` com `require_code_owner_reviews=true` e `require_last_push_approval=true` · `npm run typecheck` ✅ · `npm run build` ✅ |

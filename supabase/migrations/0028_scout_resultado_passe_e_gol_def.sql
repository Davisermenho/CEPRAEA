-- CEPR-0085 ajustes finais: resultados de passe e semântica de gol em defesa
--
-- [0028-a] Adiciona ERRO_PASSE e PASSE_INTERCEPTADO em LISTA_RESULTADO_FACTUAL.
-- [0028-b] Relaxa pontos_jogada para GIRO e AEREA: permite 1 ou 2 (execução
--          não-validada pela arbitragem para 2 pontos pode valer apenas 1).
-- [0028-c] DEF_POS e TRANS_DEF: motivo_pontuacao e pontos_jogada tornam-se
--          opcionais para GOL (gol sofrido — regra de pontuação em defesa
--          ainda não está definida no slice atual).

-- ── 1. Adicionar resultados de passe a LISTA_RESULTADO_FACTUAL ─────────────────
...
FROM (VALUES
  (
    ...
    'Erro de passe',
    17,
    'Passe ofensivo que saiu errado e encerrou a posse.',
    'Usar em AT_POS quando a sequência termina por passe claramente errado do atacante.',
    'Não usar para perda genérica ou interceptação adversária.'
  ),
  (
    'PASSE_INTERCEPTADO',
    'Passe interceptado',
    18,
    'Passe interceptado pela defesa adversária.',
    'Usar em AT_POS quando a sequência termina por interceptação defensiva do passe.',
    'Não usar para roubo de bola dominada ou perda sem passe.'
  )
...
-- ── 2. CREATE OR REPLACE create_scout_live_entry ──────────────────────────────
-- Corpo baseado na versão de migration 0027.
-- Adições sinalizadas com "[0028]"; nenhuma validação existente foi removida.
...
BEGIN
  ...
  -- Em DEF_POS e TRANS_DEF (gol sofrido), a regra de pontuação não está definida
  -- neste slice e o campo é opcional.
  ...
  IF v_entry.resultado_factual_code = 'GOL'
    ...
    -- Apenas 6M, GOLEIRA e ESPECIALISTA exigem 2 pontos.

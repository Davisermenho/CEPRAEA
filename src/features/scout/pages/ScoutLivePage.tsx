...
function EventCard({ event, onDelete }: { event: ScoutEvent; onDelete: () => void }) {
  ...
  const atletas = [...event.ataques, ...event.defesas].filter((b) => b.atleta)
    ...
    <div className="rounded-xl bg-cep-purple-900 border border-cep-purple-800 overflow-hidden">
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            {event.tempoJogo && (
              <span className="shrink-0 text-xs font-mono text-cep-lime-400 bg-cep-purple-800 rounded px-1.5 py-0.5">
                {event.tempoJogo}
              </span>
            )}
            {event.set && (
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {event.revisarVideo && (
              <Eye className="h-3.5 w-3.5 text-cep-gold-400" />
            )}
            {event.analise && (
              <Badge variant={ANALISE_VARIANT[event.analise] ?? 'gray'}>
                {event.analise}
              </Badge>
            )}
            <span className="text-xs font-bold text-cep-white">
              {event.placarCEPRAEA} – {event.placarAdversario}
            </span>
          </div>
        </div>
        {atletas.length > 0 && (
          <p className="text-xs text-cep-muted mt-1 truncate">
            {atletas.map((b) => b.atleta).join(', ')}
          </p>
        )}
      </button>
      ...
      {expanded && (
        <div className="border-t border-cep-purple-800 px-3 pb-3 pt-2 space-y-2">
          ...
          {event.ataques.filter((b) => b.atleta).length > 0 && (
            <div>
              <p className="text-xs font-semibold text-cep-lime-400 mb-1">Ataque</p>
              {event.ataques.filter((b) => b.atleta).map((b, i) => (
                <p key={i} className="text-xs text-cep-muted">
                  <span className="text-cep-white">{b.atleta}</span>
                  {b.funcao && ` · ${b.funcao}`}
                  {b.acao && ` · ${b.acao}`}
                  {b.resultadoInd && <span className="ml-1 text-cep-muted/70">({b.resultadoInd})</span>}
                </p>
              ))}
            </div>
          )}
          {event.defesas.filter((b) => b.atleta).length > 0 && (
            <div>
              <p className="text-xs font-semibold text-cep-gold-400 mb-1">Defesa</p>
              {event.defesas.filter((b) => b.atleta).map((b, i) => (
                <p key={i} className="text-xs text-cep-muted">
                  <span className="text-cep-white">{b.atleta}</span>
                  {b.funcao && ` · ${b.funcao}`}
                  {b.acao && ` · ${b.acao}`}
...
export default function ScoutLivePage() {
  ...
  // Placar atual = do último evento com placar registrado
  const lastEvent = [...events].reverse().find((e) => e.placarCEPRAEA !== undefined)
  const currentScore = {
    cepraea: lastEvent?.placarCEPRAEA ?? 0,
    adversario: lastEvent?.placarAdversario ?? 0,
  }
  const currentSet = lastEvent?.set ?? '1º SET'

  const handleSaveEvent = async (data: Omit<ScoutEvent, 'id' | 'createdAt'>) => {
    await addEvent(data)
    setFormOpen(false)
  }
  ...
  const handleReopen = async () => {
    await updateGame(game.id, { status: 'em_andamento' })
  }
    ...
    return (
      <div className="flex flex-col h-dvh bg-cep-purple-950">
        <div className="bg-cep-purple-900 border-b border-cep-purple-800 px-4 py-3 flex items-center gap-3 shrink-0">
          <button onClick={() => setFormOpen(false)} className="text-cep-muted hover:text-cep-white">
        ...
        <div className="flex-1 overflow-hidden">
          <EventForm
            jogoId={game.id}
            equipeAnalisada={game.equipeAnalisada}
            adversario={game.adversario}
            initialPlacarCEPRAEA={currentScore.cepraea}
            initialPlacarAdversario={currentScore.adversario}
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-cep-purple-900 border-b border-cep-purple-800 px-4 py-3 shrink-0">
        <div className="flex items-center gap-3 mb-3">
          <button onClick={() => navigate('/scout')} className="text-cep-muted hover:text-cep-white">
            <ArrowLeft className="h-5 w-5" />
          </button>
          <div className="flex-1 min-w-0">
            <p className="text-xs text-cep-muted truncate">{formatDate(game.data)}{game.local ? ` · ${game.local}` : ''}</p>
            <p className="text-sm font-bold text-cep-white truncate">
              {game.equipeAnalisada} <span className="font-normal text-cep-muted">vs</span> {game.adversario}
            </p>
          </div>
          <Badge variant={game.status === 'em_andamento' ? 'lime' : 'gray'}>
            {game.status === 'em_andamento' ? 'Ao vivo' : 'Finalizado'}
          </Badge>
        </div>
        ...
        <div className="flex items-center justify-center gap-6 py-2">
          <div className="text-center">
            <p className="text-xs text-cep-muted mb-0.5">{game.equipeAnalisada}</p>
            <p className="text-4xl font-black text-cep-lime-400">{currentScore.cepraea}</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-cep-muted mb-1">{currentSet}</p>
            <p className="text-2xl font-bold text-cep-muted">×</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-cep-muted mb-0.5">{game.adversario}</p>
            <p className="text-4xl font-black text-cep-white">{currentScore.adversario}</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 mt-2">
          ...
          {game.status === 'em_andamento' ? (
            <>
              ...
              <Button size="sm" variant="warning" onClick={handleFinalize}>
                <Flag className="h-4 w-4" />
                Finalizar
              </Button>
            </>
          ) : (
            <Button size="sm" variant="secondary" fullWidth onClick={handleReopen}>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {events.length === 0 ? (
          <EmptyState
            ...
            description={game.status === 'em_andamento' ? 'Toque em "Registrar Evento" para começar.' : undefined}
          />
        ) : (
          <div className="space-y-2">
            <p className="text-xs text-cep-muted">{events.length} evento{events.length !== 1 ? 's' : ''}</p>
            {[...events].reverse().map((event) => (
              <EventCard
                key={event.id}
                event={event}
                onDelete={() => setDeleteTarget(event.id)}
              />
            ))}
          </div>
        )}
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (deleteTarget) await removeEvent(deleteTarget)

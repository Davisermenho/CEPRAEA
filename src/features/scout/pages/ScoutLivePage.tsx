  ...
  return (
    <div className="rounded-xl bg-cep-purple-900 border border-cep-purple-800 overflow-hidden">
      <button className="w-full text-left p-3" onClick={() => setExpanded((v) => !v)}>
        ...
        {atletas.length > 0 && <p className="text-xs text-cep-muted mt-1 truncate">{atletas.map((b) => b.atleta).join(', ')}</p>}
      </button>
      ...
      {expanded && (
        <div className="border-t border-cep-purple-800 px-3 pb-3 pt-2 space-y-2">
          ...
          {event.ladoAcao && <p className="text-xs text-cep-muted">Lado: <span className="text-cep-white">{event.ladoAcao}</span></p>}
          {event.ataques.filter((b) => b.atleta).length > 0 && (
            <div>
              ...
              {event.ataques.filter((b) => b.atleta).map((b, i) => (
                <p key={i} className="text-xs text-cep-muted"><span className="text-cep-white">{b.atleta}</span>{b.funcao && ` · ${b.funcao}`}{b.acao && ` · ${b.acao}`}{b.resultadoInd && <span className="ml-1 text-cep-muted/70">({b.resultadoInd})</span>}</p>
              ))}
            </div>
          )}
          ...
          {event.especialistaCentral?.origemBola && <p className="text-xs text-cep-muted">Especialista: <span className="text-cep-white">{event.especialistaCentral.origemBola}</span>{event.especialistaCentral.momentoAtaque && ` · ${event.especialistaCentral.momentoAtaque}`}</p>}
...
export default function ScoutLivePage() {
  ...
  const currentScore = { cepraea: lastEvent?.placarCEPRAEA ?? 0, adversario: lastEvent?.placarAdversario ?? 0 }
      ...
      <div className="bg-cep-purple-900 border-b border-cep-purple-800 px-4 py-3 shrink-0">
        ...
        <div className="flex gap-2 mt-2">
          ...
          {game.status === 'em_andamento' ? (
            <>
              <Button size="sm" fullWidth onClick={() => setFormOpen(true)}><Plus className="h-4 w-4" />Registrar Evento</Button>
              <Button size="sm" variant="warning" onClick={handleFinalize}><Flag className="h-4 w-4" />Finalizar</Button>
            </>
          ) : (
            <Button size="sm" variant="secondary" fullWidth onClick={handleReopen}><CheckCircle className="h-4 w-4" />Reabrir Jogo</Button>
          )}
        </div>
      </div>

      <div className="flex-1 overflow-y-auto p-4">
        {events.length === 0 ? <EmptyState icon={Flag} title="Nenhum evento registrado" description={game.status === 'em_andamento' ? 'Toque em "Registrar Evento" para começar.' : undefined} /> : (
          <div className="space-y-2">
            <p className="text-xs text-cep-muted">{events.length} evento{events.length !== 1 ? 's' : ''}</p>
            {[...events].reverse().map((event) => <EventCard key={event.id} event={event} onDelete={() => setDeleteTarget(event.id)} />)}
          </div>
        )}
      </div>

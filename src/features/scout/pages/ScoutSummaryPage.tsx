...
export default function ScoutSummaryPage() {
  ...
  const shootoutEvents = events.filter((e) =>
    e.faseJogo === 'Shoot-out' ||
    e.faseJogoCEPRAEA === 'Shoot-out' ||
    e.faseJogoAdversaria === 'Shoot-out' ||
    e.shootout?.resultadoShootout
  )
  ...
  const giroErro = countWhere(events, (e) =>
    (e.resultadoColetivo ?? '').includes('giro 1 ponto') ||
    e.finalizacao?.validadeTecnica === 'Erro técnico'
  )

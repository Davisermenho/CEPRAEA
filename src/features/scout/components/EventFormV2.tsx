...
import {
  ...
  getAcoesPorFuncao,
  getCategoriaPorAcao,
  actionIndicatesFinish,
} from '../constants'

type Step = 'montagem' | 'acoes' | 'detalhes' | 'fechamento'
type Team = 'CEPRAEA' | 'Adversária'
...
const isFilledBlock = (b: ScoutAthleteBlock) => !!(b.atleta || b.numero || b.funcao || b.acao)

function isOffensivePhase(phase?: string) {
  return !!phase && (phase.includes('Ataque') || phase.includes('Transição ofensiva') || phase === 'Shoot-out')
}

function isDefensivePhase(phase?: string) {
  return !!phase && (phase.includes('Defesa') || phase.includes('Transição defensiva'))
}

function validateEventContext(form: EventFormData): string | null {
  if (!form.posse || !form.faseJogoCEPRAEA || !form.faseJogoAdversaria) return 'Informe posse, fase do CEPRAEA e fase da adversária.'

  if (form.posse === 'CEPRAEA') {
    if (!isOffensivePhase(form.faseJogoCEPRAEA)) return 'A posse está marcada como CEPRAEA, mas o CEPRAEA não está em fase ofensiva.'
    if (!isDefensivePhase(form.faseJogoAdversaria)) return 'A posse está marcada como CEPRAEA, mas a adversária não está em fase defensiva.'
  }

  if (form.posse === 'Adversário') {
    if (!isDefensivePhase(form.faseJogoCEPRAEA)) return 'A posse está marcada como Adversária, mas o CEPRAEA não está em fase defensiva.'
    if (!isOffensivePhase(form.faseJogoAdversaria)) return 'A posse está marcada como Adversária, mas a adversária não está em fase ofensiva.'
  }
...
function SelectField({ label, value, onChange, options, placeholder }: {
  ...
  return (
    <div className="space-y-1">
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-lg bg-cep-purple-800 border border-cep-purple-700 text-cep-white text-sm px-3 h-9 focus:outline-none focus:ring-2 focus:ring-cep-lime-400">
...
function TextInput({ label, value, onChange, placeholder, type = 'text' }: {
  ...
  return (
    <div className="space-y-1">
      <label className="block text-xs font-medium text-cep-muted">{label}</label>
      <input type={type} placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-lg bg-cep-purple-800 border border-cep-purple-700 text-cep-white text-sm px-3 h-9 focus:outline-none focus:ring-2 focus:ring-cep-lime-400" />
    </div>
  )
}
...
function InlineAlert({ children }: { children: React.ReactNode }) {
  return <div className="rounded-lg border border-red-500/50 bg-red-500/10 p-3 text-xs font-medium text-red-200">{children}</div>
}
...
function ParticipantSlot({ index, block, mode, funcoes, onChange }: {
  index: number
    ...
    <div className="grid grid-cols-[44px_1fr] gap-2 items-end rounded-lg border border-cep-purple-800 bg-cep-purple-950/40 p-2">
      ...
      <div className="grid grid-cols-1 gap-2">
        {mode === 'cepraea' ? (
          ...
          <TextInput label="Nº adversária" value={block.numero ?? ''} onChange={(v) => set('numero', v.replace(/\D/g, '').slice(0, 3))} placeholder="Ex.: 12, 77, 99" />
        )}
...
function ActionSlot({ index, block, mode, type, onChange }: {
  index: number
  block: ScoutAthleteBlock
  mode: 'cepraea' | 'adversario'
  type: 'ataque' | 'defesa'
  ...
  return (
    <div className="rounded-xl border border-cep-purple-700 bg-cep-purple-900/60 p-3 space-y-2">
      <p className="text-xs font-bold text-cep-lime-400 uppercase tracking-wide">{index + 1}. {label || 'Participante'}{block.funcao ? ` · ${block.funcao}` : ''}</p>
      <SelectField
        label="Ação executada"
        value={block.acao ?? ''}
        onChange={(v) => onChange({ ...block, acao: v || undefined, categoria: getCategoriaPorAcao(v, block.funcao, type) })}
        options={getAcoesPorFuncao(block.funcao, type)}
      />
      {block.categoria && <p className="text-[11px] text-cep-muted">Categoria automática: <span className="text-cep-white">{block.categoria}</span></p>}
      <SelectField label="Resultado individual" value={block.resultadoInd ?? ''} onChange={(v) => onChange({ ...block, resultadoInd: v || undefined })} options={SCOUT_RESULTADO_INDIVIDUAL} />
    </div>
  )
}
...
export function EventForm({ jogoId, equipeAnalisada, adversario, initialPlacarCEPRAEA = 0, initialPlacarAdversario = 0, initialSet, onSave, onCancel }: Props) {
  ...
  const [forceFinalizacao, setForceFinalizacao] = useState(false)
  ...
  const attackingTeam: Team = form.posse === 'CEPRAEA' ? 'CEPRAEA' : 'Adversária'
  const defendingTeam: Team = attackingTeam === 'CEPRAEA' ? 'Adversária' : 'CEPRAEA'
  const contextError = validateEventContext(form)
  ...
  const setEspecialista = (key: keyof SpecialistCentralAnalysis, val: string) => setForm((f) => ({ ...f, especialistaCentral: { ...(f.especialistaCentral ?? {}), equipe: attackingTeam, [key]: val || undefined } }))
  const setFinalizacao = (key: keyof FinishAnalysis, val: string | boolean) => setForm((f) => ({ ...f, finalizacao: { ...(f.finalizacao ?? {}), equipeFinalizadora: attackingTeam, equipeGoleira: defendingTeam, [key]: val || undefined } }))
  ...
  const finalizadoraOptions = attackingTeam === 'CEPRAEA'
    ? (form.ataqueCEPRAEA ?? []).filter((b) => b.atleta).map((b) => b.atleta!)
    : (form.ataqueAdversario ?? []).filter((b) => b.numero).map((b) => `Adversária nº ${b.numero}`)
  const goalkeeperOptions = defendingTeam === 'CEPRAEA' ? SCOUT_GOLEIRAS : ['Goleira adversária', 'Adversária nº 1', 'Adversária nº 12', 'Adversária nº 99']
  const specialistTitle = attackingTeam === 'CEPRAEA' ? 'Especialista central CEPRAEA' : 'Especialista central adversária'

  const nextStep = () => {
    if (activeStep === 'montagem' && !contextIsValid) return
    const idx = steps.indexOf(activeStep)
    setStep(steps[Math.min(idx + 1, steps.length - 1)])
  }

  const suggestPointsFromResult = (result: string) => {
    const points = result.includes('2 pontos') ? 2 : result.includes('1 ponto') ? 1 : 0
    setField('resultadoColetivo', result)
    if (points > 0) {
      setField('pontosCEPRAEA', attackingTeam === 'CEPRAEA' ? points : 0)
      setField('pontosAdversario', attackingTeam === 'Adversária' ? points : 0)
    }
  }
  ...
  const handleSave = () => {
    ...
    const cepAttack = (form.ataqueCEPRAEA ?? []).filter(isFilledBlock)
    const cepDefense = (form.defesaCEPRAEA ?? []).filter(isFilledBlock)
    const advAttack = (form.ataqueAdversario ?? []).filter(isFilledBlock)
    const advDefense = (form.defesaAdversaria ?? []).filter(isFilledBlock)
    const ataques = attackingTeam === 'CEPRAEA' ? cepAttack : advAttack
    const defesas = defendingTeam === 'CEPRAEA' ? cepDefense : advDefense
    onSave({
      jogoId,
      ...form,
      pontosCEPRAEA,
      ...
      especialistaCentral: hasCentralEspecialista ? { ...(form.especialistaCentral ?? {}), equipe: attackingTeam } : form.especialistaCentral,
      finalizacao: hasFinalizacao ? { ...(form.finalizacao ?? {}), houveFinalizacao: true, equipeFinalizadora: attackingTeam, equipeGoleira: defendingTeam } : form.finalizacao,
    })
  }

  const renderParticipants = () => {
    if (!contextIsValid) return <InlineAlert>{contextError}</InlineAlert>
    if (attackingTeam === 'CEPRAEA') {
      return (
        <>
  ...
  const renderActions = () => {
    const attackSource = attackingTeam === 'CEPRAEA' ? (form.ataqueCEPRAEA ?? []) : (form.ataqueAdversario ?? [])
    const defenseSource = defendingTeam === 'CEPRAEA' ? (form.defesaCEPRAEA ?? []) : (form.defesaAdversaria ?? [])
    const attackMode = attackingTeam === 'CEPRAEA' ? 'cepraea' : 'adversario'
    const defenseMode = defendingTeam === 'CEPRAEA' ? 'cepraea' : 'adversario'
    const updateAttackKey = attackingTeam === 'CEPRAEA' ? 'ataqueCEPRAEA' : 'ataqueAdversario'
    const updateDefenseKey = defendingTeam === 'CEPRAEA' ? 'defesaCEPRAEA' : 'defesaAdversaria'
    return (
      <>
        <Section title={`Ações do ataque ${attackingTeam === 'CEPRAEA' ? equipeAnalisada : adversario}`}>
          {attackSource.filter(isFilledBlock).map((b, i) => <ActionSlot key={i} index={i} block={b} mode={attackMode} type="ataque" onChange={(nb) => updateArray(updateAttackKey, attackSource.indexOf(b), nb)} />)}
        </Section>
        <Section title={`Ações da defesa ${defendingTeam === 'CEPRAEA' ? equipeAnalisada : adversario}`}>
          {defenseSource.filter(isFilledBlock).map((b, i) => <ActionSlot key={i} index={i} block={b} mode={defenseMode} type="defesa" onChange={(nb) => updateArray(updateDefenseKey, defenseSource.indexOf(b), nb)} />)}
        </Section>
      </>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {activeStep === 'detalhes' && (
          <>
            <label className="flex items-center gap-3 rounded-lg border border-cep-purple-800 bg-cep-purple-900/50 p-3 cursor-pointer">
              <input type="checkbox" checked={forceFinalizacao} onChange={(e) => { setForceFinalizacao(e.target.checked); setFinalizacao('houveFinalizacao', e.target.checked) }} className="h-4 w-4 rounded accent-cep-lime-400" />
              <span className="text-sm text-cep-white">Houve finalização neste evento</span>
            </label>
            {hasFinalizacao && (
              <Section title="Finalização / goleira" description={`Finalizadora: ${attackingTeam}. Goleira: ${defendingTeam}.`}>
                <SelectField label="Finalizadora" value={form.finalizacao?.finalizadora ?? ''} onChange={(v) => setFinalizacao('finalizadora', v)} options={finalizadoraOptions} />
                ...
                <SelectField label="Resultado da finalização" value={form.finalizacao?.resultadoFinalizacao ?? ''} onChange={(v) => { setFinalizacao('resultadoFinalizacao', v); suggestPointsFromResult(v) }} options={SCOUT_RESULTADO_COLETIVO} />
                <SelectField label={defendingTeam === 'CEPRAEA' ? 'Goleira CEPRAEA' : 'Goleira adversária'} value={form.finalizacao?.goleira ?? form.goleira ?? ''} onChange={(v) => { setFinalizacao('goleira', v); setField('goleira', v) }} options={goalkeeperOptions} />
            ...
            {hasCentralEspecialista && (
              <Section title={specialistTitle} description="Aparece apenas quando a equipe que ataca usa Central Especialista ou sistema com ESP central.">
            ...
            {isShootout && (
              <Section title="Shoot-out" description="Aparece apenas quando a fase é Shoot-out.">
                ...
                <SelectField label="Resultado do shoot-out" value={form.shootout?.resultadoShootout ?? ''} onChange={(v) => { setShootout('resultadoShootout', v); suggestPointsFromResult(v) }} options={SCOUT_RESULTADO_COLETIVO} />
              </Section>
            )}
          </>
        )}
        {activeStep === 'fechamento' && (
          <>
            <Section title="Resumo do evento" description="Confira o sentido do lance antes de salvar.">
              <p className="text-xs text-cep-muted">Posse: <span className="text-cep-white">{form.posse || '—'}</span></p>
              <p className="text-xs text-cep-muted">Ataque: <span className="text-cep-white">{attackingTeam === 'CEPRAEA' ? equipeAnalisada : adversario}</span></p>
              <p className="text-xs text-cep-muted">Defesa: <span className="text-cep-white">{defendingTeam === 'CEPRAEA' ? equipeAnalisada : adversario}</span></p>
              <p className="text-xs text-cep-muted">Sistema CEPRAEA: <span className="text-cep-white">{form.sistemaTaticoCEPRAEA || '—'}</span></p>
              <p className="text-xs text-cep-muted">Sistema adversária: <span className="text-cep-white">{form.sistemaTaticoAdversaria || '—'}</span></p>
              {form.finalizacao?.finalizadora && <p className="text-xs text-cep-muted">Finalizadora: <span className="text-cep-white">{form.finalizacao.finalizadora}</span></p>}
            </Section>
            ...
            <SelectField label="Resultado coletivo" value={form.resultadoColetivo ?? ''} onChange={suggestPointsFromResult} options={SCOUT_RESULTADO_COLETIVO} />
      ...
      <div className="shrink-0 p-4 border-t border-cep-purple-800 flex gap-3">
        ...
        {activeStep !== 'fechamento'
          ? <Button fullWidth onClick={nextStep} disabled={activeStep === 'montagem' && !contextIsValid}>Próximo</Button>
          : <Button fullWidth onClick={handleSave} disabled={!contextIsValid}>Salvar Evento</Button>}
      </div>
    </div>
  )
}

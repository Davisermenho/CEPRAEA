    ...
    <div className="min-h-svh w-full overflow-y-auto bg-[#14001F] text-white">
      <main className="mx-auto flex min-h-svh w-full max-w-[390px] flex-col px-[26px] pb-[max(36px,calc(env(safe-area-inset-bottom)+36px))] pt-[52px]">
        ...
        <section className="flex flex-col gap-4" aria-label="Tipos de acesso">
          <Link
            to="/atleta/login"
            className="group flex min-h-[56px] items-center justify-between rounded-xl border border-white bg-white/[0.02] px-6 py-3.5 transition hover:border-[#7EFF00] hover:bg-white/[0.05] focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-4 focus-visible:outline-[#7EFF00]/35"
          >
            <span className="text-left">
              <span className="block text-base font-medium leading-tight text-white">Sou atleta</span>
              <span className="mt-1 block text-[12px] leading-tight text-[#C02CFF]">
                Acessar treinos e informar presenças.
              </span>
            </span>
            <ChevronRight className="h-5 w-5 shrink-0 text-white transition group-hover:translate-x-0.5" aria-hidden />
          </Link>
